...
@app.get("/predictions/{location_id}", response_model=list[PredictionResponse])
def get_predictions(
    location_id: int,
...
def predict_future_demand(request: FuturePredictionRequest):
    ...
    future_trained_through = pd.Timestamp(metadata["trained_through"])
    forecast_datetime = request.forecast_datetime
    nyc_timezone = ZoneInfo("America/New_York")
    if forecast_datetime.tzinfo is not None:
        forecast_datetime = (
            forecast_datetime.astimezone(nyc_timezone).replace(tzinfo=None)
        )
    ...
    if not profile["profile_rows"]:
        raise HTTPException(
            status_code=404,
            detail=(
                "No future demand profile found "
    ...
    if profile["exact_demand"] is not None:
        ...
        forecast_method = str(metadata["production_model"])
    elif profile["month_hour_demand"] is not None:
        predicted_demand = float(profile["month_hour_demand"])
        forecast_method = "month_hour_fallback"
    elif profile["hour_demand"] is not None:
        predicted_demand = float(profile["hour_demand"])
        forecast_method = "hour_fallback"
    else:
